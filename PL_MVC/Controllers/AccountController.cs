...
namespace PL_MVC.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        public ActionResult Register()
        {
            ...
            model.Usuario.Direccion = new ML.Direccion();
            model.Usuario.Direccion.Colonia = new ML.Colonia();
            model.Usuario.Direccion.Colonia.Municipio=new ML.Municipio();
            model.Usuario.Direccion.Colonia.Municipio.Estado = new ML.Estado();
            ML.Result resultRol = BL.Rol.GetAllEF();
            model.Usuario.Rol = new ML.Rol();
            model.Usuario.Rol.Roles = resultRol.Objects;
            ML.Result resultEstado = BL.Estado.GetAllEF();
            model.Usuario.Direccion.Colonia.Municipio.Estado.Estados = resultEstado.Objects;
            return View(model); 
        }     
        ...
        public async Task<ActionResult> Register(RegisterViewModel model, HttpPostedFileBase file)
        {
            if (ModelState.IsValid)
            {
                ...
                if (result.Succeeded)
                {
                    ...
                    if (file != null)
                    {
                        model.Usuario.Imagen = ConvertToBase64(file);
                    }
                    model.Usuario.Email = model.Email;
                    model.Usuario.UserName = model.Email;
                    model.Usuario.Password = model.Password;
                    model.Usuario.IdIdentity = user.Id;

                    System.Net.ServicePointManager.ServerCertificateValidationCallback = (senderX, certificate, chain, sslPolicyErrors) => { return true; };

                    ML.Result resultUsuario = Add(model.Usuario);
                    var currentUser = UserManager.FindByName(user.UserName);
                    var roleresult = UserManager.AddToRole(currentUser.Id, model.Usuario.Rol.RolNombre);
